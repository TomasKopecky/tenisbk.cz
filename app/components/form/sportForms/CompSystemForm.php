...
class CompSystemForm extends BasicForms// tzv. továrna pro zpracování formulářů v sekci úpravy a vkládání registrací {
{
    ...
    public function create($operation, $presenter, $compSystem = null)
    { // základní metoda s parametry pro obslužnou metodu formuláře (update, insert)
        ...
        $form->addCheckbox('alternativni_system_zapasu', 'Alternativní systém zápasů');
    ...
    {
        if (
            $form['id_soutez']->getSelectedItem() == $this->compSystem->getCompetition()->getName() &&
            ...
            $form['alternativni_system_zapasu']->getValue() == $this->compSystem->getAlternativeMatchSystem()
        ) {
    ...
    public function setCompetitionsAssoc($competitions)
    { // převede získanou tabulku hráčů z databáze na asoc. pole formát "id_hrac" => "jmeno"
    ...
    protected function setDefaults($form)
    {
        $form->setDefaults([
            ...
            'alternativni_system_zapasu' => $this->compSystem->getAlternativeMatchSystem()
        ]);
    }

    public function update($form)
    { //provede se po odeslání vyplněného formuláře pro úpravu hráče
        try {
            ...
            //$this->presenter->flashMessage('Databázová chyba při úpravě systému. Změny vráceny zpět.', 'error');
    ...
    { //provede se po odeslání vyplněného formuláře pro vložení nového hráče
        try {
            $values = $form->getValues();
