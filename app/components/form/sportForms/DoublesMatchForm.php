...
class DoublesMatchForm extends BasicForms {
    ...
    public function getPlayersList() {
        ...
        if (strpos($this->presenter->getAction(), 'zapasyNovy') !== false) {
            $sex = strpos($this->presenter->getAction(), 'Muzi') != false ? 'M' : 'X';
        }
        if (strpos($this->presenter->getAction(), 'zapasyUprava') !== false) {

            $sex = $this->matchesTable->getMatchTypeId() == 2 ? 'M' : 'X';
        }
    ...
    private function setPlayersSelect($form) {
        ...
        $form['hrac2_domaci']->setItems($this->setPlayersAssoc($this->playersHome->getPlayersList()));
        $form['hrac2_hoste']->setItems($this->setPlayersAssoc($this->playersVisitors->getPlayersList()));
    }
    ...
    private function setPlayersInputs($form) {
        ...
        $form['hrac1_domaci']->addCondition($form::NOT_EQUAL, 'null')
                ->addRule(Form::NOT_EQUAL, 'Zadali jste stejného hráče domácích dvakrát', $form['hrac2_domaci']);
        $form['hrac1_hoste']->addCondition($form::NOT_EQUAL, 'null')
                ->addRule(Form::NOT_EQUAL, 'Zadali jste stejného hráče hostů dvakrát', $form['hrac2_hoste']);
    }
        ...
        foreach ($inputs as $input) {
            for ($i = 1; $i < 4; $i++) {
                $form->addText($input . $i . '_domaci')
                        ...
                        ->setHtmlType('number');
                //->addRule(Form::MIN, 'Zadejte hodnotu od 0 do 7', 0);
                ...
                //$form['set' . $i . '_domaci']->addRule(Form::MAX, 'Zadejte hodnotu od 0 do 7', 7);
                //$form['set' . $i . '_hoste']->addRule(Form::MAX, 'Zadejte hodnotu od 0 do 7', 7);
            }
        }
    }
    ...
    public function validateResults($form) {
        ...
        if ($checkResult->getHomeLossDefault() || $checkResult->getVisitorsLossDefault()) {
            ...
            $form['hrac2_domaci']->setValue(null);
            $form['hrac2_hoste']->setValue(null);
        }
    }
    ...
    public function validateForm($form) {
        if (
                ...
                $form['hrac2_domaci']->getValue() == $this->matchesTable->getPlayerHome2()->getId() &&
                $form['hrac2_hoste']->getValue() == $this->matchesTable->getPlayerVisitors2()->getId() &&
    ...
    protected function setDefaults($form) {
        ...
        $form->setDefaults([
            ...
            'hrac2_domaci' => $this->matchesTable->getPlayerHome2()->getId(),
            'hrac2_hoste' => $this->matchesTable->getPlayerVisitors2()->getId(),
    ...
    public function update($form) { //provede se po odeslání vyplněného formuláře pro úpravu hráče
        try {
            ...
            $this->mailer->setMessage('Chyba - úprava zápasu čtyřhry', 'Nastala nespecifikovaná chyba při editaci zápasu čtyřhry - úpravě. Jednalo se o zápas s id ' . $this->presenter->getParameter('id') . ' mezi hráči ' . $form['hrac1_domaci']->getValue() . ', ' . $form['hrac2_domaci']->getValue() . 'a ' . $form['hrac1_hoste']->getValue() . ', ' . $form['hrac2_hoste']->getValue() . '. Text chyby: ' . $e->getMessage());
...
//$form->addError('Chyba při vkládání registrace. Nespecifikovaný typ, kontaktujte správce.');
            $this->mailer->setMessage('Chyba - vkládní zápasu čtyřhry', 'Nastala nespecifikovaná chyba při editaci zápasu čtyřhry - vkládání. Jednalo se o zápas mezi hráči ' . $form['hrac1_domaci']->getValue() . ', ' . $form['hrac2_domaci']->getValue() . 'a ' . $form['hrac1_hoste']->getValue() . ', ' . $form['hrac2_hoste']->getValue() . '. Text chyby: ' . $e->getMessage());
            $this->mailer->setSenderReceiver("error@tenisbk.cz", "admin@tenisbk.cz");
            $this->mailer->sendMessage();
